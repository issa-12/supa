...
- **Content moderation**: `CommunityService.moderateAndAnalyze` uses the Anthropic API (`claude-haiku-4-5`) to classify each post `approved` / `flagged` / `rejected` + sentiment. **Policy: both `rejected` (hate/harassment/explicit/off-topic spam) and `flagged` (actual profanity / direct personal insult / spam) are blocked at publish time (422), so only `approved` posts are stored.** **Narrowed (Post-Day 15):** the prompt rules were loosened so normal book discussion — including *harsh, negative, or passionate criticism* of a book/author/plot, debates, and frustration — is **approved**; the model is told to "Default to approved" and that "a negative or critical book opinion is always approved." This was a deliberate fix because the earlier rules (which flagged "mildly inappropriate, spam-like, very mild rudeness") were too aggressive and blocked ordinary opinionated reviews. `flagged` now means *actual profanity, a direct insult aimed at a person, or obvious spam/advertising* — so bad-word posts are still blocked, but critical reviews pass. The feed filter (`getAllPosts`/`getTrendingPosts`) also hides any pre-existing `flagged`/`rejected` rows: `or(moderation_status.is.null,and(...neq.rejected,...neq.flagged))` (legacy null-status posts stay visible). Note: moderation runs only at post time — Claude still classifies profanity as `flagged` (not `rejected`), which is why blocking `flagged` (not just `rejected`) remains required to stop bad-word posts. Every failure path (missing `ANTHROPIC_API_KEY`, API error, JSON parse error) falls back to `approved`/`neutral` so posting never hard-fails on an AI outage. **Frontend error UX:** all three composers (home `posts-feed`, `post-comments`, community page) map the 422 to a short, translated `contentRejected` message (EN/AR/FR) — they never display the AI's raw `reason` text.
